...
namespace AssetKits.ParticleImage
{
    ...
    public class ParticleTrailRenderer : MaskableGraphic
    {
        ...
        public void PrepareMeshData(int vertexCount, int particleCount)
        {
            ...
            {
                Debug.LogWarning($"vertexCount ({vertexCount}) is less than particleCount ({particleCount}). Clamping values.");
        ...
        public void UpdateMeshData(NativeArray<Vector3> points, NativeArray<int> tris, NativeArray<Color> cols)
        {
            ...
            {
                Debug.LogError($"Vertex buffer overflow: {offset + points.Length} > {vertexBuffer.Length}");
                return;
            }

            if (offset + cols.Length > colorBuffer.Length)
            {
                Debug.LogError($"Color buffer overflow: {offset + cols.Length} > {colorBuffer.Length}");
                return;
            }

            if (trisOffset + tris.Length > indexBuffer.Length)
            {
                Debug.LogError($"Index buffer overflow: {trisOffset + tris.Length} > {indexBuffer.Length}");
                return;
            }
